...
export default function BusGuardHero() {
  ...
  useEffect(() => {
    ...
    function buildHousing() {
      ...
      addBox(W, TH, D, 0, -H / 2 + TH / 2, 0, shell);
      addBox(TH, H, D, -W / 2 + TH / 2, 0, 0, shell);
      addBox(TH, H, D, W / 2 - TH / 2, 0, 0, shell);
      addBox(W, H, TH, 0, 0, -D / 2 + TH / 2, shell);
      addBox(W, H, TH, 0, 0, D / 2 - TH / 2, shell);
      const eW = 0.03;
      ...
      const aLine = M(B(W, 0.018, 0.018), accent); aLine.position.set(0, H / 2 - 0.009, D / 2); g.add(aLine);
      const aBot = M(B(W, 0.018, 0.018), accent); aBot.position.set(0, -H / 2 + 0.009, D / 2); g.add(aBot);
      const camRecess = M(B(0.52, 0.40, 0.14), inner); camRecess.position.set(0.50, 0.04, D / 2 + 0.03); g.add(camRecess);
      const camRing = M(T(0.19, 0.018, 10, 32), accent); camRing.rotation.x = Math.PI / 2; camRing.position.set(0.50, 0.04, D / 2 + 0.01); g.add(camRing);
      const lensShadow = M(new THREE.CircleGeometry(0.17, 28), inner); lensShadow.rotation.x = Math.PI / 2; lensShadow.position.set(0.50, 0.04, D / 2 + 0.065); g.add(lensShadow);
      const piAp = M(B(0.055, 0.16, 0.24), inner); piAp.position.set(-W / 2 + 0.015, 0.06, 0.08); g.add(piAp);
      const piFrame = M(T(0.08, 0.012, 8, 20), accent); piFrame.rotation.z = Math.PI / 2; piFrame.position.set(-W / 2, 0.06, 0.08); g.add(piFrame);
      const tray = M(B(W - 0.2, 0.05, D - 0.2), inner); tray.position.y = -H / 2 + TH + 0.025; g.add(tray);
      for (let x = -0.75; x <= 0.75; x += 0.38) { const ch = M(B(0.038, 0.032, D - 0.25), S(0x111116, 0.85, 0.05)); ch.position.set(x, -H / 2 + TH + 0.04, 0); g.add(ch); }
      [[-W / 2 + 0.2, -H / 2 + TH, D / 2 - 0.2], [W / 2 - 0.2, -H / 2 + TH, D / 2 - 0.2],
       [-W / 2 + 0.2, -H / 2 + TH, -D / 2 + 0.2], [W / 2 - 0.2, -H / 2 + TH, -D / 2 + 0.2]].forEach(p => {
        const boss = M(C(0.04, 0.04, 0.1, 10), S(0x303038, 0.5, 0.3)); boss.position.set(...p); g.add(boss);
        const screw = M(C(0.018, 0.018, 0.05, 8), S(0xaaa870, 0.2, 0.92)); screw.position.set(p[0], p[1] + 0.075, p[2]); g.add(screw);
      });
      ...
      for (let i = 0; i < 5; i++) { const v = M(B(0.32, 0.028, 0.018), inner); v.position.set(-0.5 + i * 0.26, 0.2, -D / 2 + 0.005); g.add(v); }
      const plate = M(B(0.7, 0.14, 0.005), S(0xeeeeea, 0.85, 0.02)); plate.position.set(-0.6, -0.35, D / 2 + 0.002); g.add(plate);
      return g;
    }
    ...
    function buildPCB() {
      ...
      const heatSpreader = M(B(0.28, 0.009, 0.28), S(0x8a8870, 0.12, 0.95)); heatSpreader.position.set(-0.24, 0.066, 0.02); g.add(heatSpreader);
      for (let r = 0; r < 4; r++) for (let c = 0; c < 4; c++) { const bump = M(C(0.007, 0.007, 0.012, 6), goldMat); bump.position.set(-0.34 + c * 0.055, 0.038, -0.065 + r * 0.055); g.add(bump); }
      ...
      [-0.22, 0.22].forEach(z => {
        ...
        const rimM = M(B(0.005, 0.078, 0.068), S(0xd0d0c0, 0.2, 0.9)); rimM.position.set(0.825, 0.062, z); g.add(rimM);
      });
      const eth = M(B(0.148, 0.095, 0.235), S(0xb8a468, 0.38, 0.45)); eth.position.set(0.91, 0.075, 0.43); g.add(eth);
      const ethCav = M(B(0.105, 0.062, 0.19), S(0x444452, 0.6)); ethCav.position.set(0.94, 0.075, 0.43); g.add(ethCav);
      [{ c: 0xffaa00, z: 0.32 }, { c: 0x00ff44, z: 0.52 }].forEach(({ c, z }) => { const led = M(B(0.018, 0.018, 0.005), S(c, 0.2, 0, { emissive: c, emissiveIntensity: 3 })); led.position.set(0.9, 0.095, z); g.add(led); });
      ...
      [{ c: 0x00ff55, x: 0.70, z: -0.39 }, { c: 0xff4400, x: 0.59, z: -0.39 }].forEach(({ c, x, z }) => { const led = M(new THREE.SphereGeometry(0.016, 10, 10), S(c, 0.2, 0, { emissive: c, emissiveIntensity: 3.5 })); led.position.set(x, 0.038, z); g.add(led); });
      ...
      [[0.3, 0, -0.32], [0.55, 0, 0.08], [-0.5, 0, 0.28], [0.15, 0, 0.34]].forEach(([x, , z]) => {
        ...
        const capTop = M(new THREE.CircleGeometry(0.019, 10), S(0x888870, 0.2, 0.8)); capTop.rotation.x = -Math.PI / 2; capTop.position.set(x, 0.078, z); g.add(capTop);
      });
    ...
    function buildBuzzerSet() {
      ...
      positions.forEach((off) => {
        ...
        const faceMesh = M(C(0.098, 0.098, 0.005, 24), S(0x28282e, 0.6, 0.18)); faceMesh.position.y = 0.044; bg.add(faceMesh);
    ...
    const PIECES = [
      ...
      { obj: varifocal, key: "varifocal", name: "1080p Varifocal Camera", final: { p: [0.50, 0.04, 0.38],    r: [0, -Math.PI / 2, 0] }, start: { p: [6.5, 1.5, 6],         r: [0, -Math.PI / 2 + 0.5, 0.18] } },
    ...
    function triggerTitleIntro() {
      ...
      void titleWrap.offsetWidth; // reflow
      setTimeout(() => {
        titleWrap.style.animation = ""; // clear inline override so class animation fires
        titleWrap.classList.add("bgh-slide-in");
      }, 200);
    ...
    function triggerReveal() {
      ...
      revealWord.classList.remove("bgh-big-pop");
      revealSub.classList.remove("bgh-show");
    ...
    function hideReveal() {
      ...
      revealWrap.classList.remove("bgh-reveal-show");
      revealWord.classList.remove("bgh-big-pop");
      revealSub.classList.remove("bgh-show");
    }
    ...
    function tick() {
      ...
      const dt = Math.min(now - prevTime, 0.035);
      prevTime = now;
      ...
      PIECES.forEach(({ obj, start, final, delay }, i) => {
        if (phase === "assembling" || phase === "holding") {
          ...
          obj.position.set(
            lerp(start.p[0], final.p[0], ease),
            lerp(start.p[1], final.p[1], ease),
            lerp(start.p[2], final.p[2], ease)
          );
          obj.rotation.set(
            lerp(start.r[0], final.r[0], ease),
            lerp(start.r[1], final.r[1], ease),
            lerp(start.r[2], final.r[2], ease)
          );
        ...
        if (phase === "exploding" || phase === "revealing") {
          ...
          obj.position.set(
            lerp(final.p[0], ex.p[0], ease),
            lerp(final.p[1], ex.p[1], ease),
            lerp(final.p[2], ex.p[2], ease)
          );
          obj.rotation.set(
            lerp(final.r[0], ex.r[0], ease),
            lerp(final.r[1], ex.r[1], ease),
            lerp(final.r[2], ex.r[2], ease)
          );
        }
      });
      ...
      else if (phase === "assembling") { sceneRotY = lerp(sceneRotY, 0, 0.025); }
